...
def _render_stack_overview(tool_calls: pd.DataFrame, session_stats: pd.DataFrame):
    ...
    if not lang_dist.empty:
        st.subheader("Language Distribution", help="File reads, writes, and edits broken down by file extension/language.")
        top = lang_dist.head(15)
        fig = go.Figure()
        fig.add_trace(go.Bar(y=top["language"], x=top["reads"], name="Reads", orientation="h", marker_color="#636EFA"))
        fig.add_trace(go.Bar(y=top["language"], x=top["writes"], name="Writes", orientation="h", marker_color="#EF553B"))
        fig.add_trace(go.Bar(y=top["language"], x=top["edits"], name="Edits", orientation="h", marker_color="#00CC96"))
        fig.update_layout(barmode="group", height=400, yaxis=dict(autorange="reversed"), margin=dict(l=0, r=0, t=60, b=0))
        st.plotly_chart(fig, use_container_width=True)

    # ── Language Trend Over Time
    if not lang_daily.empty:
        st.subheader("Language Trend Over Time", help="Weekly file activity per language. Shows which languages dominate work over time.")
    ...
    with col_eco:
        st.subheader("Ecosystem Signals", help="Detected runtime/ecosystem keywords (e.g. node_modules, venv, .gradle) across all file paths.")
        if not eco_signals.empty:
            ...
            fig.update_layout(height=300, showlegend=False, margin=dict(l=0, r=0, t=10, b=0), yaxis=dict(autorange="reversed"))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No ecosystem signals detected")

    with col_fw:
        st.subheader("Frameworks Detected", help="Frameworks inferred from file patterns and import paths (e.g. React, FastAPI, Django).")
    ...
    with col_layer:
        st.subheader("Development Layer Split", help="Frontend vs Backend vs Infrastructure file split, based on path and extension heuristics.")
        if not layers.empty:
    ...
    with col_db:
        st.subheader("Database Signals", help="Database technologies detected from file paths, query strings, and import patterns.")
        if not db_signals.empty:
            fig = px.bar(db_signals, x="signal_count", y="database", orientation="h", color="database")
            fig.update_layout(height=350, showlegend=False, margin=dict(l=0, r=0, t=10, b=0), yaxis=dict(autorange="reversed"))
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    if not profiles.empty:
        st.divider()
        st.subheader("Project Profiles", help="Per-project summary: primary language, frameworks detected, and layer distribution.")
        st.dataframe(
            profiles,
            use_container_width=True,
            hide_index=True,
...
def _render_libraries(tool_calls: pd.DataFrame, session_stats: pd.DataFrame):
    ...
    st.subheader("Most Installed Packages", help="Most frequently installed packages across all sessions, grouped by package manager.")
    top = installs.head(20)
    fig = px.bar(top, x="count", y="package", orientation="h", color="pkg_manager",
                 color_discrete_sequence=px.colors.qualitative.Set2)
    fig.update_layout(height=max(300, len(top) * 25), yaxis=dict(autorange="reversed"),
                      margin=dict(l=0, r=0, t=60, b=0))
    st.plotly_chart(fig, use_container_width=True)

    # Timeline
    if not timeline.empty:
        st.subheader("Install Activity Over Time", help="Daily package install commands over time, broken down by package manager.")
    ...
    if not by_project.empty:
        ...
        st.dataframe(
            by_project, use_container_width=True, hide_index=True,
            column_config={
                "project": "Project",
                "packages": "Packages",
...
def _render_testing(tool_calls: pd.DataFrame, session_stats: pd.DataFrame):
    ...
    # Frequency chart
    if not frequency.empty:
        st.subheader("Test Frequency Over Time", help="Daily count of test runner invocations (pytest, jest, cargo test, etc.).")
        frequency["date"] = pd.to_datetime(frequency["date"])
        fig = px.bar(frequency, x="date", y="test_count",
    ...
    with col_fw:
        st.subheader("Test Frameworks", help="Distribution of test frameworks detected from Bash commands (pytest, jest, mocha, etc.).")
        if not frameworks.empty:
    ...
    with col_pos:
        st.subheader("Test Timing", help="Whether tests were run before or after code changes — a proxy for TDD discipline.")
        if not position.empty:
            fig = px.pie(position, values="count", names="position", hole=0.4,
...
def _render_research(tool_calls: pd.DataFrame):
    ...
    # Volume line chart
    if not volume.empty:
        st.subheader("Research Volume Over Time", help="Daily WebSearch, WebFetch, and Grep tool calls — indicators of research and information-gathering activity.")
    ...
    with col_q:
        st.subheader("Top Search Queries", help="Most frequent search query strings passed to WebSearch across all sessions.")
        if not topics.empty:
            top = topics.head(15)
            fig = px.bar(top, x="count", y="query", orientation="h",
                         color_discrete_sequence=["#636EFA"])
            fig.update_layout(height=max(300, len(top) * 25),
    ...
    with col_d:
        st.subheader("Documentation Sources", help="Top domains fetched via WebFetch — shows which documentation sites are most referenced.")
        if not sources.empty:
            top = sources.head(15)
            fig = px.bar(top, x="count", y="domain", orientation="h",
    ...
    if not grep_pats.empty:
        st.subheader("Top Grep Patterns", help="Most common regex patterns used in Grep/ripgrep tool calls — reveals what code structures are frequently searched.")
        st.dataframe(
            grep_pats.head(20), use_container_width=True, hide_index=True,
            column_config={
                "pattern": "Pattern",
...
def _render_edit_semantics(tool_calls: pd.DataFrame):
    ...
    with col_cat:
        st.subheader("Edit Categories", help="Semantic classification of edits: feature additions, bug fixes, refactors, config changes, etc.")
        fig = px.pie(categories, values="count", names="category", hole=0.4)
        fig.update_layout(height=350, margin=dict(l=0, r=0, t=10, b=0))
        st.plotly_chart(fig, use_container_width=True)

    with col_cplx:
        st.subheader("Edit Complexity", help="Average line-delta per edit operation over time, overlaid with total edit count. Higher delta = larger changes.")
        if not complexity.empty:
            complexity["date"] = pd.to_datetime(complexity["date"])
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=complexity["date"], y=complexity["avg_delta"],
                                     name="Avg Delta", mode="lines+markers"))
            fig.add_trace(go.Bar(x=complexity["date"], y=complexity["edit_count"],
                                 name="Edit Count", opacity=0.3))
            fig.update_layout(height=350, margin=dict(l=0, r=0, t=10, b=0))
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    if not timeline.empty:
        st.subheader("Change Types Over Time", help="Weekly breakdown of edit categories — shows how the mix of features, fixes, and refactors evolves.")
    ...
    if not imports.empty:
        st.subheader("Import Additions by Language", help="Count of new import/require statements added across sessions, by language — tracks dependency growth.")
        fig = px.bar(imports, x="import_count", y="language", orientation="h",
                     color_discrete_sequence=["#00CC96"])
        fig.update_layout(height=max(250, len(imports) * 30),
                          yaxis=dict(autorange="reversed"),
                          margin=dict(l=0, r=0, t=10, b=0))
        st.plotly_chart(fig, use_container_width=True)
