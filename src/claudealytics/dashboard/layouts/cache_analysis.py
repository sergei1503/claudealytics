"""Cache Analysis tab: deep dive into per-conversation cache efficiency."""
...
def render(stats):
    ...
    if not daily_metrics.empty:
        ...
        with col1:
            date_from = st.date_input("From", value=daily_metrics["date"].min(), key="cache_from")
        with col2:
            date_to = st.date_input("To", value=daily_metrics["date"].max(), key="cache_to")

        date_mask = (daily_metrics["date"] >= pd.to_datetime(date_from)) & (
            daily_metrics["date"] <= pd.to_datetime(date_to)
        )
        filtered_daily = daily_metrics[date_mask]

        cost_mask = (cost_savings["date"] >= pd.to_datetime(date_from)) & (
            cost_savings["date"] <= pd.to_datetime(date_to)
        )
    ...
    if not filtered_daily.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            y=filtered_daily["cache_hit_rate"],
            mode="lines+markers",
            name="Cache Hit Rate",
        ...
        fig.add_hline(y=70, line_dash="dash", line_color="rgba(239,68,68,0.5)",
                       annotation_text="70% healthy threshold")
        fig.update_layout(
    ...
    if not filtered_daily.empty:
        ...
        fig.add_trace(go.Scatter(
            x=filtered_daily["date"],
            y=filtered_daily["cache_reuse_multiplier"],
            mode="lines+markers",
            name="Reuse Multiplier",
            line=dict(color="#8b5cf6", width=2),
            marker=dict(size=5),
            hovertemplate="Date: %{x|%Y-%m-%d}<br>Multiplier: %{y:.2f}x<extra></extra>",
        ))
        fig.add_hline(y=1.25, line_dash="dash", line_color="rgba(245,158,11,0.5)",
                       annotation_text="1.25x break-even")
        fig.update_layout(
            height=350, margin=dict(l=20, r=20, t=40, b=0),
    ...
    # --- TTL Distribution ---
    if not filtered_daily.empty and (
        filtered_daily["ephemeral_1h_tokens"].sum() > 0
        or filtered_daily["ephemeral_5m_tokens"].sum() > 0
    ):
        ...
        fig.add_trace(go.Scatter(
            x=filtered_daily["date"],
            y=filtered_daily["ephemeral_1h_tokens"],
            mode="lines+markers",
            name="1h Ephemeral",
        ...
        fig.add_trace(go.Scatter(
            x=filtered_daily["date"],
            y=filtered_daily["ephemeral_5m_tokens"],
            mode="lines+markers",
            name="5m Ephemeral",
    ...
    if not filtered_costs.empty:
        st.subheader("Cache Cost Savings", help="Estimated daily USD savings from cache reads vs paying full input token price. Calculated as (no-cache cost) - (actual cost).")
        ...
        fig.add_trace(go.Bar(
            x=filtered_costs["date"],
            y=filtered_costs["no_cache_cost"],
            name="Without Cache",
    ...
    if not project_summary.empty:
        st.subheader("Cache Efficiency by Project", help="Average cache hit rate per project. Red < 50%, amber 50-70%, green > 70%. Low rates may indicate frequent model or tool list changes.")

        proj_df = project_summary.copy()
        # Color code: red <50%, amber 50-70%, green >70%
        proj_df["color"] = proj_df["avg_hit_rate"].apply(
            lambda x: "#ef4444" if x < 50 else ("#f59e0b" if x < 70 else "#22c55e")
        )

        fig = go.Figure()
        fig.add_trace(go.Bar(
            y=proj_df["project"],
            x=proj_df["avg_hit_rate"],
            orientation="h",
            marker_color=proj_df["color"],
            text=proj_df["avg_hit_rate"].apply(lambda x: f"{x:.1f}%"),
            textposition="outside",
            hovertemplate=(
                "Project: %{y}<br>"
                "Avg Hit Rate: %{x:.1f}%<br>"
                "<extra></extra>"
            ),
        ))
        fig.add_vline(x=70, line_dash="dash", line_color="rgba(100,100,100,0.3)")
        fig.update_layout(
            height=max(200, len(proj_df) * 40 + 80),
            margin=dict(l=20, r=80, t=20, b=0),
            xaxis_title="Avg Cache Hit Rate (%)",
            xaxis=dict(range=[0, 105]),
        )
        st.plotly_chart(fig, use_container_width=True)

        # Show project details table
        st.dataframe(
            ...
            .rename(columns={
                ...
                "avg_reuse_multiplier": "Avg Reuse Multiplier",
    ...
    if not breaking.empty:
        with st.expander("Cache-Breaking Sessions"):
            ...
            display_cols = [
                "date", "project", "model", "message_count",
                "cache_hit_rate", "cache_reuse_multiplier", "reasons",
            ]
            ...
            st.dataframe(
                display_df.rename(columns={
                    ...
                    "cache_reuse_multiplier": "Reuse Mult.",
                    "reasons": "Reasons",
                }),
                hide_index=True,
                use_container_width=True,
    ...
    st.divider()
    st.caption(
        "Cache pricing: cache read = 10% of input price, "
        "cache creation = 125% (5m TTL) or 200% (1h TTL) of input price. "
        "Reuse multiplier > 1.25 means cache paid for itself. "
        "Cache is prefix-based: tools -> system prompt -> CLAUDE.md -> conversation."
    )


def _render_kpis(daily_metrics: pd.DataFrame, cost_savings: pd.DataFrame, session_df: pd.DataFrame):
    """Render the 4-metric KPI header row."""
    ...
    if not daily_metrics.empty:
        ...
        total_all_input = (
            daily_metrics["input_tokens"].sum()
            + total_cache_read
            + daily_metrics["cache_creation_input_tokens"].sum()
        )
