...
from claudealytics.analytics.cost_calculator import MODEL_PRICING, DEFAULT_PRICING
...
def compute_daily_cache_metrics(daily_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if daily_df.empty:
        return pd.DataFrame(columns=[
            "date", "cache_hit_rate", "cache_reuse_multiplier",
            "ephemeral_1h_tokens", "ephemeral_5m_tokens",
            "cache_read_input_tokens", "cache_creation_input_tokens",
            "input_tokens",
        ])

    grouped = daily_df.groupby("date").agg(
    ...
    total_input = (
        ...
        + grouped["cache_read_input_tokens"]
        + grouped["cache_creation_input_tokens"]
    )
    grouped["cache_hit_rate"] = (
        grouped["cache_read_input_tokens"] / total_input.replace(0, float("nan")) * 100
    ).fillna(0).round(2)

    grouped["cache_reuse_multiplier"] = (
        grouped["cache_read_input_tokens"]
        / grouped["cache_creation_input_tokens"].replace(0, float("nan"))
    ).fillna(0).round(2)
...
def compute_cost_savings(daily_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if daily_df.empty:
        return pd.DataFrame(columns=[
            "date", "actual_cost", "no_cache_cost", "savings_usd", "savings_pct",
        ])
...
def compute_session_cache_metrics(session_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["total_input_tokens"] = (
        df["input_tokens"] + df["cache_read_input_tokens"] + df["cache_creation_input_tokens"]
    )
...
def detect_cache_breaking_sessions(
    session_df: pd.DataFrame, min_messages: int = 3
) -> pd.DataFrame:
...
def project_cache_summary(session_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if session_df.empty:
        return pd.DataFrame(columns=[
            "project", "session_count", "avg_hit_rate",
            "avg_reuse_multiplier", "model_switch_pct",
        ])

    grouped = session_df.groupby("project").agg(
    ...
    grouped["model_switch_pct"] = (
        grouped["model_switch_count"] / grouped["session_count"] * 100
    ).round(1)
    grouped["avg_hit_rate"] = grouped["avg_hit_rate"].round(1)
    grouped["avg_reuse_multiplier"] = grouped["avg_reuse_multiplier"].round(2)

    return grouped.drop(columns=["model_switch_count"]).sort_values(
        "avg_hit_rate", ascending=False
    )
